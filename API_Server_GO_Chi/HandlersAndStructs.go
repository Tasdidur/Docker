...
func (p *Product) changeCount(c int) (ok bool){
	ok = false
	if p.Brand == "" || p.Model=="" || p.Category==""{
		return
	}
	for i, j := range products {
		if matchIncompleteCompleteProducts(*p, j) {
			if products[i].Count+c < 0 {
				return
			}
			ok = true
			products[i].Count += c
...
func (p *Product) deleteProduct() (ok bool){
	ok = false
	for i, j := range products {
		if matchIncompleteCompleteProducts(*p, j) {
...
func deleteProduct(w http.ResponseWriter, r *http.Request) {
	...
	ok := p.deleteProduct()
	if ok{
		writeResponse(w,http.StatusOK,"Product deleted successfully")
	}else {
		writeResponse(w,http.StatusBadRequest,"No such Product")
	}
}
...
func buyProduct(w http.ResponseWriter, r *http.Request) {
	...
	I.UserName = uName
	I.Product = pro
	ok := pro.changeCount(-1 * pro.Count)
	if ok == false{
		writeResponse(w,http.StatusBadRequest,"not available")
		return
	}
